...
export const ConnectJourney: React.FC<{ division: Division }> = ({ division }) => {
  ...
  return (
    <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
      <div className={`flex items-center gap-2 border-b pb-4 overflow-x-auto scrollbar-none ${isDark ? 'border-white/10' : 'border-black/10'}`}>
        {tabs.map(t => (
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
            className={`flex items-center gap-2 px-4 py-2 font-semibold text-sm transition-all rounded-lg whitespace-nowrap ${
              tab === t.id ? (isDark ? 'bg-[var(--primary-color)] text-[#1a1a1a]' : 'bg-[var(--primary-color)] text-white shadow-md') : (isDark ? 'text-slate-400 hover:text-white hover:bg-white/5' : 'text-slate-500 hover:text-black hover:bg-black/5')
            }`}
          >
